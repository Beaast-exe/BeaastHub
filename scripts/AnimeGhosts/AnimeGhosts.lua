    ...
    while task.wait() and not Library.Unloaded do
        if settings['AutoScroll']['Enabled'] then
            if not getPlayerGamemode() then
                ...
                FireBridge('PetSystem', 'Open', settings['AutoScroll']['SelectedScroll'], 'All')
            end
        end 
    end
end)
...
AutoGacha:AddToggle('enableLogSpins', {
    Text = 'Log Spins to Console',
    Default = settings['AutoSpin']['LogSpins'],

    Callback = function(value)
        settings['AutoSpin']['LogSpins'] = value
        SaveConfig()
    end
})
...
task.spawn(function()
    while task.wait(0.5) and not Library.Unloaded do
        if settings['AutoUpgrades']['CrewMastery']['Enabled'] then
            ...
            local PlayerInventory = PlayerData and PlayerData.Inventory
            local PlayerUpgrades = PlayerData and PlayerData.Upgrades
            local SelectedUpgrades = settings['AutoUpgrades']['CrewMastery']['SelectedUpgrades']
            
            local UpgradesData = require(ReplicatedStorage:WaitForChild("Framework"):WaitForChild("Modules"):WaitForChild("Data"):WaitForChild("UpgradeData"):WaitForChild("Crew Mastery"))
            local UpgradesList = UpgradesData.Targets
            local UpgradesMaxPrice = UpgradesData.MaxPrice
            local UpgradesPriceScale = UpgradesData.PriceScale
            local UpgradesCurrency = UpgradesData.Currency
...
task.spawn(function()
    while task.wait(0.5) and not Library.Unloaded do
        if settings['AutoUpgrades']['StandMastery']['Enabled'] then
            ...
            local PlayerInventory = PlayerData and PlayerData.Inventory
            local PlayerUpgrades = PlayerData and PlayerData.Upgrades
            local SelectedUpgrades = settings['AutoUpgrades']['StandMastery']['SelectedUpgrades']
            
            local UpgradesData = require(ReplicatedStorage:WaitForChild("Framework"):WaitForChild("Modules"):WaitForChild("Data"):WaitForChild("UpgradeData"):WaitForChild("Stand Mastery"))
            local UpgradesList = UpgradesData.Targets
            local UpgradesMaxPrice = UpgradesData.MaxPrice
            local UpgradesPriceScale = UpgradesData.PriceScale
            local UpgradesCurrency = UpgradesData.Currency
...
task.spawn(function()
    while task.wait(0.5) and not Library.Unloaded do
        if settings['AutoUpgrades']['FiendMastery']['Enabled'] then
            ...
            local PlayerInventory = PlayerData and PlayerData.Inventory
            local PlayerUpgrades = PlayerData and PlayerData.Upgrades
            local SelectedUpgrades = settings['AutoUpgrades']['FiendMastery']['SelectedUpgrades']
            
            local UpgradesData = require(ReplicatedStorage:WaitForChild("Framework"):WaitForChild("Modules"):WaitForChild("Data"):WaitForChild("UpgradeData"):WaitForChild("Fiend Mastery"))
            local UpgradesList = UpgradesData.Targets
            local UpgradesMaxPrice = UpgradesData.MaxPrice
            local UpgradesPriceScale = UpgradesData.PriceScale
            local UpgradesCurrency = UpgradesData.Currency
...
task.spawn(function()
    while task.wait(0.5) and not Library.Unloaded do
        if settings['AutoUpgrades']['EasterMastery']['Enabled'] then
            ...
            local PlayerInventory = PlayerData and PlayerData.Inventory
            local PlayerUpgrades = PlayerData and PlayerData.Upgrades
            local SelectedUpgrades = settings['AutoUpgrades']['EasterMastery']['SelectedUpgrades']
            
            local UpgradesData = require(ReplicatedStorage:WaitForChild("Framework"):WaitForChild("Modules"):WaitForChild("Data"):WaitForChild("UpgradeData"):WaitForChild("Easter Mastery"))
            local UpgradesList = UpgradesData.Targets
            local UpgradesMaxPrice = UpgradesData.MaxPrice
            local UpgradesPriceScale = UpgradesData.PriceScale
            local UpgradesCurrency = UpgradesData.Currency
        ...
        pcall(function()
            if settings['Exchange']['Currency']['Buy']['Enabled'] and Toggles['enableAutoExchangeBuy'].Value then
                ...
                if getItemAmount('ExchangeTokens') > 100000 and getItemAmount(currency) <= 900000 then
                    FireBridge('ExchangeSystem', 'Make', 'Currency', 'ExchangeTokens', tostring('ExchangeTokens' .. currency), 100000)
                elseif getItemAmount('ExchangeTokens') > 50000 and getItemAmount(currency) <= 950000 then
                    FireBridge('ExchangeSystem', 'Make', 'Currency', 'ExchangeTokens', tostring('ExchangeTokens' .. currency), 50000)
                elseif getItemAmount('ExchangeTokens') > 10000 and getItemAmount(currency) <= 990000 then
                    FireBridge('ExchangeSystem', 'Make', 'Currency', 'ExchangeTokens', tostring('ExchangeTokens' .. currency), 10000)
                elseif getItemAmount('ExchangeTokens') > 5000 and getItemAmount(currency) <= 995000 then
                    FireBridge('ExchangeSystem', 'Make', 'Currency', 'ExchangeTokens', tostring('ExchangeTokens' .. currency), 5000)
                elseif getItemAmount('ExchangeTokens') > 1000 and getItemAmount(currency) <= 999000 then
                    FireBridge('ExchangeSystem', 'Make', 'Currency', 'ExchangeTokens', tostring('ExchangeTokens' .. currency), 1000)
                elseif getItemAmount('ExchangeTokens') > 500 and getItemAmount(currency) <= 999500 then
                    FireBridge('ExchangeSystem', 'Make', 'Currency', 'ExchangeTokens', tostring('ExchangeTokens' .. currency), 500)
                elseif getItemAmount('ExchangeTokens') > 100 and getItemAmount(currency) <= 999900 then
                    FireBridge('ExchangeSystem', 'Make', 'Currency', 'ExchangeTokens', tostring('ExchangeTokens' .. currency), 100)
                elseif getItemAmount('ExchangeTokens') > 50 and getItemAmount(currency) <= 999950 then
                    FireBridge('ExchangeSystem', 'Make', 'Currency', 'ExchangeTokens', tostring('ExchangeTokens' .. currency), 50)
                elseif getItemAmount('ExchangeTokens') > 10 and getItemAmount(currency) <= 999990 then
                    FireBridge('ExchangeSystem', 'Make', 'Currency', 'ExchangeTokens', tostring('ExchangeTokens' .. currency), 10)
                elseif getItemAmount('ExchangeTokens') > 1  and getItemAmount(currency) <= 999999 then
                    FireBridge('ExchangeSystem', 'Make', 'Currency', 'ExchangeTokens', tostring('ExchangeTokens' .. currency), 1)
                end
            end
        end)
    end
    ...
    while task.wait(0.1) and not Library.Unloaded do
        ...
        pcall(function()
            if settings['GachaSpins']['World3']['PowerOfThePlanets'] and hasTokenAmountToRoll("ExchangeTokens") and not PlayerGachaIndex['Power of the Planet']['Sun'] and not settings['AutoSpin']['PauseSpins'] then
                ...
                if settings['AutoSpin']['LogSpins'] then
                    print("Spinning: Power of the Planets")
                end
            end
        end)
